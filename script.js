document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATA ---
    const collections = [
        { id: "beverages", name: "B", previews: ["dwater.jpg", "dcokeb.jpg", "dcokec.jpg", "dpepsi.jpg"] },
        { id: "snacks", name: "S", previews: ["ssalted.jpg", "skurkurem.jpg", "slaysg.jpg", "skurkurec.jpg"] },
        { id: "biscuits", name: "B", previews: ["bicrakjack.jpg", "biparleg.jpg", "bihideandseek.jpg", "bioreo.jpg"] },
        { id: "chocolates", name: "C", previews: ["chdc.jpg", "chcrispello.jpg", "chfruitnnut.jpg", "chkinderjoy.jpg"] },
        { id: "cleaningessentials", name: "C", previews: ["clarielb.jpg", "clharpic.jpg", "clodonill.jpg", "clfeathern.jpg"] },
        { id: "personal", name: "P", previews: ["pccomb.jpg", "pcalmond.jpg", "pcgillette.jpg", "pctowel.jpg"] }
    ];

    const products = [
        { id: 1, name: "Bottle-Pepsi", image: "dpepsi.jpg", cat: "beverages", subcat: "Cold Drink", selectedVariant: "S", variants: { "S": { price: 40, count: 0, unit: "750ml" }, "L": { price: 90, count: 0, unit: "2L" } } },
...
/* Personal */
    ...
    function startCounters() {
        ...
        function update(currentTime, startTime, counter, target) {
            const elapsed = currentTime - startTime;
            const progress = Math.min(elapsed / counterDuration, 1);
            const current = Math.floor(progress * target);
            counter.innerText = `${current}+`;
            if (progress < 1) {
                requestAnimationFrame((time) => update(time, startTime, counter, target));
            } else {
        ...
        counters.forEach(counter => {
            ...
            requestAnimationFrame((time) => update(time, time, counter, target));
        });
    }
    checkbox.addEventListener('change', () => {
    body.classList.toggle('dark-mode');
    // Optional: Add this line to save the setting
    ...
    function renderProducts(catId) {
        ...
        if (filtered.length === 0) {
            ...
            // Keeping navigation layout intact as per instructions
            return;
        }
    ...
    pgroupslider.addEventListener('click', (e) => {
        ...
        if (item) {
            ...
            if (targetElement) {
                ...
                productSlider.scrollTo({ top: targetElement.offsetTop - headerOffset, behavior: "smooth" });
            }
        }
    });
    ...
    productSlider.addEventListener('scroll', () => {
        ...
        if (productScrollTotal > 0) {
            ...
            pgroupslider.scrollTop = scrollPct * (pgroupslider.scrollHeight - pgroupslider.clientHeight);
        }
        const sections = productGrid.querySelectorAll('.card[id^="sub-"]');
        let currentSectionId = "";
        sections.forEach(sec => {
            const rect = sec.getBoundingClientRect();
            if (rect.top <= productSlider.getBoundingClientRect().top + 100) currentSectionId = sec.id;
        });
    ...
    function updateDate() {
        ...
        if (dateElement) dateElement.textContent = `📅 ${day}/${month}/${now.getFullYear()}`;
    }

    function updateCount() {
        if (countElement) countElement.textContent = Math.floor(Math.random() * 10) + 1;
    }
    ...
    function updateSidebar() {
        ...
        body.style.marginBottom = totalItems > 0 ? "7rem" : "0";
